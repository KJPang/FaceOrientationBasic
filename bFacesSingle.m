function bFacesSingle()
clear all

ID = 'test';
Condition = 'comp2' %'sizeDistance' %'presentationDelay' %'head';
% output filename
filename = ['C:\kieran\honoursweirdstuff\results3\', Condition, ID, '.txt']; % CC
...
image_dist = [65 90 90 65 90 90]; %[90 140 140 90 140 140]; %[90 140 140 90 140 140]; %[105 155 155 105 155 155]; %[150 200 200 150 200 200] %[115 165 165 115 165 165]; %[150 200 200 150 200 200]; %[200 250 250 200 250 250] %fir second condition %300.*ones(1,6); % pixels

resize_factor =  .11; %.155; %.175; %.225; %.3 %for second condition
resize_factorF = .10; %.14;
j_shift = [0, 0];
...
DrawFormattedText(win, 'On each trial, indicate which centre face is oriented more directly forwards.\n\nPress F for the left centre face; press J for the right.', 'center', 'center');
Screen('Flip',win);
...
for t = 1:num_stim
    ...
    for n = 1:num_images
        ...
        if usedtochoosering(1) > 3
            dest_rect(n,:) = [(SCREEN_X/2)-150 SCREEN_Y/2 (SCREEN_X/2)-150 SCREEN_Y/2] + [-size(I,2)/2+1 -size(I,1)/2+1 size(I,2)/2 size(I,1)/2].*resize_factor + [vector_pos(n,1) vector_pos(n,2) vector_pos(n,1) vector_pos(n,2)];
            dest_rect_shift(n,:) = [(SCREEN_X/2)-150 (SCREEN_Y/2)+v_pix (SCREEN_X/2)-150 (SCREEN_Y/2)+v_pix] + [-size(I,2)/2+1 -size(I,1)/2+1 size(I,2)/2 size(I,1)/2].*resize_factor + [vector_pos(n,1) vector_pos(n,2) vector_pos(n,1) vector_pos(n,2)];
            dest_rect2(n,:) = [(SCREEN_X/2)+150 SCREEN_Y/2 (SCREEN_X/2)+150 SCREEN_Y/2] + [-size(I,2)/2+1 -size(I,1)/2+1 size(I,2)/2 size(I,1)/2].*resize_factor + [vector_pos(n,1) vector_pos(n,2) vector_pos(n,1) vector_pos(n,2)];
            dest_rect_shift2(n,:) = [(SCREEN_X/2)+150 (SCREEN_Y/2)+v_pix (SCREEN_X/2)+150 (SCREEN_Y/2)+v_pix] + [-size(I,2)/2+1 -size(I,1)/2+1 size(I,2)/2 size(I,1)/2].*resize_factor + [vector_pos(n,1) vector_pos(n,2) vector_pos(n,1) vector_pos(n,2)];
        else
            dest_rect(n,:) = [(SCREEN_X/2)-150 SCREEN_Y/2 (SCREEN_X/2)-150 SCREEN_Y/2] + [-size(I,2)/2+1 -size(I,1)/2+1 size(I,2)/2 size(I,1)/2].*resize_factorF + [vector_pos(n,1) vector_pos(n,2) vector_pos(n,1) vector_pos(n,2)];
            dest_rect_shift(n,:) = [(SCREEN_X/2)-150 (SCREEN_Y/2)+v_pix (SCREEN_X/2)-150 (SCREEN_Y/2)+v_pix] + [-size(I,2)/2+1 -size(I,1)/2+1 size(I,2)/2 size(I,1)/2].*resize_factorF + [vector_pos(n,1) vector_pos(n,2) vector_pos(n,1) vector_pos(n,2)];
            dest_rect2(n,:) = [(SCREEN_X/2)+150 SCREEN_Y/2 (SCREEN_X/2)+150 SCREEN_Y/2] + [-size(I,2)/2+1 -size(I,1)/2+1 size(I,2)/2 size(I,1)/2].*resize_factorF + [vector_pos(n,1) vector_pos(n,2) vector_pos(n,1) vector_pos(n,2)];
            dest_rect_shift2(n,:) = [(SCREEN_X/2)+150 (SCREEN_Y/2)+v_pix (SCREEN_X/2)+150 (SCREEN_Y/2)+v_pix] + [-size(I,2)/2+1 -size(I,1)/2+1 size(I,2)/2 size(I,1)/2].*resize_factorF + [vector_pos(n,1) vector_pos(n,2) vector_pos(n,1) vector_pos(n,2)];
        end
%         if usedtochoosering(1) > 3
%             dest_rect(n,:) = [(SCREEN_X/2)-250 SCREEN_Y/2 (SCREEN_X/2)-250 SCREEN_Y/2] + [-size(I,2)/2+1 -size(I,1)/2+1 size(I,2)/2 size(I,1)/2].*resize_factor + [vector_pos(n,1) vector_pos(n,2) vector_pos(n,1) vector_pos(n,2)];
%             dest_rect_shift(n,:) = [(SCREEN_X/2)-250 (SCREEN_Y/2)+v_pix (SCREEN_X/2)-250 (SCREEN_Y/2)+v_pix] + [-size(I,2)/2+1 -size(I,1)/2+1 size(I,2)/2 size(I,1)/2].*resize_factor + [vector_pos(n,1) vector_pos(n,2) vector_pos(n,1) vector_pos(n,2)];
%             dest_rect2(n,:) = [(SCREEN_X/2)+250 SCREEN_Y/2 (SCREEN_X/2)+250 SCREEN_Y/2] + [-size(I,2)/2+1 -size(I,1)/2+1 size(I,2)/2 size(I,1)/2].*resize_factor + [vector_pos(n,1) vector_pos(n,2) vector_pos(n,1) vector_pos(n,2)];
%             dest_rect_shift2(n,:) = [(SCREEN_X/2)+250 (SCREEN_Y/2)+v_pix (SCREEN_X/2)+250 (SCREEN_Y/2)+v_pix] + [-size(I,2)/2+1 -size(I,1)/2+1 size(I,2)/2 size(I,1)/2].*resize_factor + [vector_pos(n,1) vector_pos(n,2) vector_pos(n,1) vector_pos(n,2)];
%         else
%             dest_rect(n,:) = [(SCREEN_X/2)-250 SCREEN_Y/2 (SCREEN_X/2)-250 SCREEN_Y/2] + [-size(I,2)/2+1 -size(I,1)/2+1 size(I,2)/2 size(I,1)/2].*resize_factorF + [vector_pos(n,1) vector_pos(n,2) vector_pos(n,1) vector_pos(n,2)];
%             dest_rect_shift(n,:) = [(SCREEN_X/2)-250 (SCREEN_Y/2)+v_pix (SCREEN_X/2)-250 (SCREEN_Y/2)+v_pix] + [-size(I,2)/2+1 -size(I,1)/2+1 size(I,2)/2 size(I,1)/2].*resize_factorF + [vector_pos(n,1) vector_pos(n,2) vector_pos(n,1) vector_pos(n,2)];
%             dest_rect2(n,:) = [(SCREEN_X/2)+250 SCREEN_Y/2 (SCREEN_X/2)+250 SCREEN_Y/2] + [-size(I,2)/2+1 -size(I,1)/2+1 size(I,2)/2 size(I,1)/2].*resize_factorF + [vector_pos(n,1) vector_pos(n,2) vector_pos(n,1) vector_pos(n,2)];
%             dest_rect_shift2(n,:) = [(SCREEN_X/2)+250 (SCREEN_Y/2)+v_pix (SCREEN_X/2)+250 (SCREEN_Y/2)+v_pix] + [-size(I,2)/2+1 -size(I,1)/2+1 size(I,2)/2 size(I,1)/2].*resize_factorF + [vector_pos(n,1) vector_pos(n,2) vector_pos(n,1) vector_pos(n,2)];
%         end
    end
    
    %ensure identities are not the same
    while usedtochoosering(1) == usedtochoosefaces(1);
        usedtochoosefaces = randperm(6);
    end
    
    %even more face stuff
    ...
    if usedtochoosefaces(1) > 3
        dest2rect = [(SCREEN_X/2)-150 SCREEN_Y/2 (SCREEN_X/2)-150 SCREEN_Y/2] + [-size(I,2)/2+1 -size(I,1)/2+1 size(I,2)/2 size(I,1)/2].*resize_factor;
        dest2rect_shift = [(SCREEN_X/2)-150 (SCREEN_Y/2)+v_pix (SCREEN_X/2)-150 (SCREEN_Y/2)+v_pix] + [-size(I,2)/2+1 -size(I,1)/2+1 size(I,2)/2 size(I,1)/2].*resize_factor;
        dest2rect2 = [(SCREEN_X/2)+150 SCREEN_Y/2 (SCREEN_X/2)+150 SCREEN_Y/2] + [-size(I,2)/2+1 -size(I,1)/2+1 size(I,2)/2 size(I,1)/2].*resize_factor;
        dest2rect_shift2 = [(SCREEN_X/2)+150 (SCREEN_Y/2)+v_pix (SCREEN_X/2)+150 (SCREEN_Y/2)+v_pix] + [-size(I,2)/2+1 -size(I,1)/2+1 size(I,2)/2 size(I,1)/2].*resize_factor;
    else
        dest2rect = [(SCREEN_X/2)-150 SCREEN_Y/2 (SCREEN_X/2)-150 SCREEN_Y/2] + [-size(I,2)/2+1 -size(I,1)/2+1 size(I,2)/2 size(I,1)/2].*resize_factorF;
        dest2rect_shift = [(SCREEN_X/2)-150 (SCREEN_Y/2)+v_pix (SCREEN_X/2)-150 (SCREEN_Y/2)+v_pix] + [-size(I,2)/2+1 -size(I,1)/2+1 size(I,2)/2 size(I,1)/2].*resize_factorF; 
        dest2rect2 = [(SCREEN_X/2)+150 SCREEN_Y/2 (SCREEN_X/2)+150 SCREEN_Y/2] + [-size(I,2)/2+1 -size(I,1)/2+1 size(I,2)/2 size(I,1)/2].*resize_factorF;
        dest2rect_shift2 = [(SCREEN_X/2)+150 (SCREEN_Y/2)+v_pix (SCREEN_X/2)+150 (SCREEN_Y/2)+v_pix] + [-size(I,2)/2+1 -size(I,1)/2+1 size(I,2)/2 size(I,1)/2].*resize_factorF;       
    end
%     if usedtochoosefaces(1) > 3
    ...
    if flipIntervals(1) == 1
        if whichway(1) == 1
            for jj=1:stim_durn_frames
                Screen('DrawTexture', win, theface, [], dest2rect, [], 0, cont(jj));
                Screen('DrawTextures', win, stim, [], dest_rect', [], 0, cont(jj));
                Screen('DrawTexture', win, theface2, [], dest2rect_shift2, [], 0, cont(jj));
                Screen('DrawTextures', win, flop, [], dest_rect_shift2', [], 0, cont(jj));
                Screen('DrawDots', win, [(SCREEN_X-fix_size)/2 (SCREEN_Y-fix_size)/2], fix_size, black,[0,0],1);
                vbl_new = Screen('Flip',win); %,vbl_old+(1/120));
                %vbl_old = vbl_new;
            end
        ...
        elseif whichway(1) == 2
            for jj=1:stim_durn_frames
                ...
                Screen('DrawTextures', win, flop, [], dest_rect', [], 0, cont(jj));
                Screen('DrawTexture', win, theface2, [], dest2rect_shift2, [], 0, cont(jj));
                Screen('DrawTextures', win, stim, [], dest_rect_shift2', [], 0, cont(jj));
                Screen('DrawDots', win, [(SCREEN_X-fix_size)/2 (SCREEN_Y-fix_size)/2], fix_size, black,[0,0],1);
                vbl_new = Screen('Flip',win); %,vbl_old+(1/120));
                %vbl_old = vbl_new;
            end
    ...
    elseif flipIntervals(1) == 2
        if whichway(1) == 1
            for kk=1:stim_durn_frames
                ...
                Screen('DrawTexture', win, theface, [], dest2rect2, [], 0, cont(kk));
                Screen('DrawTextures', win, stim, [], dest_rect2', [], 0, cont(kk));
                Screen('DrawDots', win, [(SCREEN_X-fix_size)/2 (SCREEN_Y-fix_size)/2], fix_size, black,[0,0],1);
                Screen('Flip',win);
            end
            test_head2
        elseif whichway(1) == 2
            for kk=1:stim_durn_frames
                ...
                Screen('DrpawTexture', win, theflip, [], dest2rect2, [], 0, cont(kk));
                Screen('DrawTextures', win, flop, [], dest_rect2', [], 0, cont(kk));
                Screen('DrawDots', win, [(SCREEN_X-fix_size)/2 (SCREEN_Y-fix_size)/2], fix_size, black,[0,0],1);
                Screen('Flip',win);
            end
            %test_head2  
