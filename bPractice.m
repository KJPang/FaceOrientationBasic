...
function bPractice(subjID,subjVert,oriMean,acq,relativeSurr)

% TI_psi_2int(subjID,subjVert,oriMean,acq,relativeSurr)
%
% e.g. bPractice('K',0,0,1,0);
%
...
[I, ~, alpha] = imread(['C:\Kieran\honoursweirdstuff\dudepractice\3a.bmp']);
pracst5 = Screen('MakeTexture',window,I);
[I, ~, alpha] = imread(['C:\Kieran\honoursweirdstuff\dudepractice\3b.bmp']);
pracst6 = Screen('MakeTexture',window,I);
[I, ~, alpha] = imread(['C:\Kieran\honoursweirdstuff\dudepractice\4a.bmp']);
pracst7 = Screen('MakeTexture',window,I);
[I, ~, alpha] = imread(['C:\Kieran\honoursweirdstuff\dudepractice\4b.bmp']);
pracst8 = Screen('MakeTexture',window,I);
[I, ~, alpha] = imread(['C:\Kieran\honoursweirdstuff\dudepractice\5a.bmp']);
pracst9 = Screen('MakeTexture',window,I);
[I, ~, alpha] = imread(['C:\Kieran\honoursweirdstuff\dudepractice\5b.bmp']);
pracst10 = Screen('MakeTexture',window,I);
...
%set shifted rects
dest_rect = [(SCREEN_X/2)-150 SCREEN_Y/2 (SCREEN_X/2)-150 SCREEN_Y/2] + [-size(I,2)/2+1 -size(I,1)/2+1 size(I,2)/2 size(I,1)/2];%.*resize_factor;% + [vector_pos(n,1) vector_pos(n,2) vector_pos(n,1) vector_pos(n,2)];
dest2_rect = [(SCREEN_X/2)+150 SCREEN_Y/2 (SCREEN_X/2)+150 SCREEN_Y/2] + [-size(I,2)/2+1 -size(I,1)/2+1 size(I,2)/2 size(I,1)/2];%.*resize_factor;% + [vector_pos(n,1) vector_pos(n,2) vector_pos(n,1) vector_pos(n,2)];
...
DrawFormattedText(window,'On each trial, indicate whether the face on the left or right is oriented more directly forwards. \n\nPress F for the face on the left; press J for the right.',...
...
%         newEvent('Stim off',[],GetSecs - the_experiment_start - eventLog{eventCounter-1,1}); %get length of stimulus duration
    ...
    for fix=1:fix_frames
        ...
        if fristorsecodn == 1
            for jj=1:stim_durn_frames
                Screen('DrawDots', window, [(SCREEN_X-fix_size)/2 (SCREEN_Y-fix_size)/2], fix_size, [0, 0, 0],[0,0],1);
                Screen('DrawTexture', window, pracst1, [], dest_rect, [], 0, cont(jj));
                Screen('DrawTexture', window, pracst2, [], dest2_rect, [], 0, cont(jj));
                Screen('Flip',window,[],[],[],1);
            end
        elseif fristorsecodn == 3
            for jj=1:stim_durn_frames
                Screen('DrawDots', window, [(SCREEN_X-fix_size)/2 (SCREEN_Y-fix_size)/2], fix_size, [0, 0, 0],[0,0],1);
                Screen('DrawTexture', window, pracst3, [], dest_rect, [], 0, cont(jj));
                Screen('DrawTexture', window, pracst4, [], dest2_rect, [], 0, cont(jj));
                Screen('Flip',window,[],[],[],1);
            end
        elseif fristorsecodn == 5
            for jj=1:stim_durn_frames
                Screen('DrawDots', window, [(SCREEN_X-fix_size)/2 (SCREEN_Y-fix_size)/2], fix_size, [0, 0, 0],[0,0],1);
                Screen('DrawTexture', window, pracst5, [], dest_rect, [], 0, cont(jj));
                Screen('DrawTexture', window, pracst6, [], dest2_rect, [], 0, cont(jj));
                Screen('Flip',window,[],[],[],1);
                switch1 = 1;
            end
        elseif fristorsecodn == 7
            for jj=1:stim_durn_frames
                Screen('DrawDots', window, [(SCREEN_X-fix_size)/2 (SCREEN_Y-fix_size)/2], fix_size, [0, 0, 0],[0,0],1);
                Screen('DrawTexture', window, pracst7, [], dest_rect, [], 0, cont(jj));
                Screen('DrawTexture', window, pracst8, [], dest2_rect, [], 0, cont(jj));
                Screen('Flip',window,[],[],[],1);
            end
        elseif fristorsecodn == 9
            for jj=1:stim_durn_frames
                Screen('DrawDots', window, [(SCREEN_X-fix_size)/2 (SCREEN_Y-fix_size)/2], fix_size, [0, 0, 0],[0,0],1);
                Screen('DrawTexture', window, pracst9, [], dest_rect, [], 0, cont(jj));
                Screen('DrawTexture', window, pracst10, [], dest2_rect, [], 0, cont(jj));
                Screen('Flip',window,[],[],[],1);
                switch1 = 2;
            end
        end
    ...
    while ~keypress
        ...
        if switch1 == 1;
            if response == 1;
                DrawFormattedText(window, 'Correct Response \n \n The centre face on the left was more direct, so the F key should have been pressed', 'center', 'center');
            else
                DrawFormattedText(window, 'Incorrect Response \n \n The centre face on the left was more direct, so the F key should have been pressed', 'center', 'center');
            end
        else
            if response == 2;
                DrawFormattedText(window, 'Correct Response \n \n The centre face on the right was more direct, so the J key should have been pressed', 'center', 'center');
            else
                DrawFormattedText(window, 'Incorrect Response \n \n The centre face on the right was more direct, so the J key should have been pressed', 'center', 'center');
            end
        end
